...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    <div
      className={`card-nav-container fixed left-1/2 -translate-x-1/2 z-[99] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isScrolled ? 'w-[80%] max-w-[900px] top-[0.8em]' : 'w-[95%] max-w-[1200px] top-[1.2em] md:top-[2em]'} ${className}`}
    >
      <nav
        ref={navRef}
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] md:pl-[1.1rem] z-[2]">
          ...
          <div className="flex items-center gap-2 order-3 md:order-none overflow-visible z-10">
            <Link href="/#kontakt" className="group relative overflow-hidden inline-flex h-9 md:h-10 items-center justify-center rounded-lg bg-sapphire px-3 md:px-5 text-sm font-semibold text-white shadow transition-all duration-300 hover:bg-sapphire-hover hover:shadow-[0_4px_16px_rgba(15,82,186,0.5)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50">
              {/* Shine effect */}
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="nav-card-label font-semibold tracking-[-0.5px] text-[16px] md:text-[18px]" style={{ fontFamily: 'var(--font-syne)' }}>
                {item.label}
              </div>
