...
export default function Footer() {
    ...
    // Social-URLs: nur anzeigen wenn echte URL gesetzt (Platzhalter # blendet aus)
    const socials = [
    ...
    return (
        <footer className="relative pt-32 pb-12 px-6 overflow-hidden">
            ...
            <div className="relative max-w-5xl mx-auto flex flex-col items-center text-center">
                {/* Social Icons — nur sichtbar wenn ENV-URL gesetzt */}
                {socials.length > 0 && (
                    <div className="flex items-center gap-3 mb-10">
                        {socials.map((s) => (
                            <a
                                ...
                                className="w-10 h-10 rounded-full border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.04] flex items-center justify-center hover:border-sapphire/30 hover:bg-sapphire/5 transition-all duration-300"
                            >
                                {s.label === "X" && (
