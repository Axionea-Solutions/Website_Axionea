...
const tags = [
    { label: "Arztpraxen", icon: "M22 12h-4l-3 9L9 3l-3 9H2", key: true },
    { label: "Kieferorthopäden", icon: "M12 5.5c-2-2-5-2.5-6 .5-1 3 1 6 2.5 9 .8 1.6 1.5 3 3.5 3s2.7-1.4 3.5-3c1.5-3 3.5-6 2.5-9-1-3-4-2.5-6-.5z", key: true },
    { label: "Immobilienmakler", icon: "M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z", key: true },
...
function TagItem({ label, icon, isKey }: { label: string; icon: string; isKey?: boolean }) {
    return (
        <span
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full border text-sm whitespace-nowrap transition-all duration-300 shrink-0 ${
                isKey
...
export default function IndustryTagsBand() {
    return (
        <section className="py-12 md:py-16 px-6 border-t border-b border-black/5 dark:border-white/5">
            {/* Header — Fokus benennen, Bandbreite zeigen */}
            <div className="max-w-3xl mx-auto text-center mb-8">
                ...
                <p className="text-muted-foreground text-base md:text-lg">
                    Unser Fokus: <span className="text-foreground font-medium">Arztpraxen, Kieferorthopäden, Makler &amp; Steuerberater</span>. Die Technik dahinter funktioniert in vielen weiteren Branchen.
                </p>
            </div>
            ...
            <div className="relative max-w-7xl mx-auto overflow-hidden rounded-2xl">
                ...
                <div className="flex w-max animate-scroll-left hover:[animation-play-state:paused]">
                    ...
                    <div className="flex gap-4 shrink-0 pr-4">
                        {tags.map((tag) => (
                            <TagItem key={`a-${tag.label}`} label={tag.label} icon={tag.icon} isKey={tag.key} />
                        ))}
                    </div>
                    {/* Track 2 */}
                    <div className="flex gap-4 shrink-0 pr-4">
                        {tags.map((tag) => (
                            <TagItem key={`b-${tag.label}`} label={tag.label} icon={tag.icon} isKey={tag.key} />
                        ))}
                    </div>
                    {/* Track 3 */}
                    <div className="flex gap-4 shrink-0 pr-4">
                        {tags.map((tag) => (
                            <TagItem key={`c-${tag.label}`} label={tag.label} icon={tag.icon} isKey={tag.key} />
                        ))}
                    </div>
                </div>
            </div>
